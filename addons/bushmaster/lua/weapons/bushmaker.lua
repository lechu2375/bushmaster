...
if (SERVER) then
	...
	function SWEP:Reload()
		if (areaPoint.startVector and areaPoint.endVector) then
			...
			net.WriteUInt(BUSH_CREATE,3)
			net.WriteVector(areaPoint.startVector)
			net.WriteVector(areaPoint.endVector)
			net.SendToServer()
			areaPoint.startVector = nil
			areaPoint.endVector = nil
		end
	end
	...
	function SWEP:DrawHUD()
		local w, h = ScrW(), ScrH()
		local cury = h/4*3
		local tx, ty = draw.SimpleText("Left Click: Set Area Point", "DermaLarge", w/2, cury, color_white, 1, 1)
		cury = cury + ty
		local tx, ty = draw.SimpleText("Right Click: Reset Area Point", "DermaLarge", w/2, cury, color_white, 1, 1)
		cury = cury + ty
		local tx, ty = draw.SimpleText("Reload: Register Area", "DermaLarge", w/2, cury, color_white, 1, 1)
